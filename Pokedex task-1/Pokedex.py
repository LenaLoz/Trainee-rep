...
def top_matches(name_pokemon):
    ...
    try:
        for x in pokemon_data.keys():
            matches = find_near_matches(subsequence=name_pokemon, sequence=x, max_l_dist=1,
                                        max_deletions=1,
                                        max_insertions=1,
                                        max_substitutions=1
                                        )
            if matches:
                matched_names.append(x)
        ...
        raise ValueError(f'No matches was found({e}')
